...
object MonteCarloPricingExample extends Example {
  ...
  val r=0.02
  val discountCurves=Map[Currency,Double=>Double](USD->(t=>r))
  def values = cashflows map(c=>value(c, t0, discountCurves))
  // Output the results
  object outputScenarios
  object outputTransactionsLists
  object outputCashflows
  object outputValues
  val outputSelector=outputValues
  def outputResult(s:Any) {
    s match {
      case outputScenarios => output(outputScenarios)
      case outputTransactionsLists => transactionsLists |>>> Iteratee.foreach(output)
      case outputCashflows => cashflows |>>> Iteratee.foreach(output)
      case outputValues => println(">>>"); values |>>> Iteratee.foreach(output)
    }
  }
  output("Transactions")
